...
use crate::events::{
    ...
    emit_participant_filter_mode_changed, emit_recurring_lock_cancelled,
    emit_recurring_lock_created, emit_recurring_lock_executed, emit_risk_flags_updated,
    emit_ticket_claimed, emit_ticket_issued, emit_timelock_configured, AdminActionCancelled,
    AdminActionExecuted, AdminActionProposed, BatchFundsLocked, BatchFundsReleased,
    ...
    NotificationPreferencesUpdated, ParticipantFilterModeChanged, RecurringLockCancelled,
    RecurringLockCreated, RecurringLockExecuted, RefundTriggerType, RiskFlagsUpdated,
    TicketClaimed, TicketIssued, TimelockConfigured, EVENT_VERSION_V2,
};
...
pub struct PendingAction {
    ...
    pub payload: Bytes, // Serialized ActionPayload
...
pub enum Error {
    ...
    EscrowFrozen = 45,
    AddressFrozen = 46,
    TimelockNotEnabled = 47,
    TimelockNotElapsed = 48,
    TimelockEnabled = 49,
    ActionNotFound = 50,
    ActionAlreadyExecuted = 51,
    ActionAlreadyCancelled = 52,
    InvalidPayload = 53,
...
impl BountyEscrowContract {
    ...
    fn route_fee(
        ...
        let mut total_weight: u64 = 0;
        for dest in config.treasury_destinations.iter() {
            total_weight = total_weight
                .checked_add(dest.weight as u64)
                .unwrap_or(u64::MAX);
        }
    ...
    pub fn propose_admin_action(
        ...
        let action_id: u64 = env
            .storage()
            .instance()
            .get::<_, u64>(&DataKey::ActionCounter)
            .unwrap_or(0u64)
            .checked_add(1)
            .unwrap_or(0);
        ...
        let serialized_payload = payload.to_xdr(&env);
    ...
    pub fn execute_after_delay(env: Env, action_id: u64) -> Result<(), Error> {
        ...
        // Deserialize payload
        let payload =
            ActionPayload::from_xdr(&env, &action.payload).map_err(|_| Error::ActionNotFound)?;
    ...
    pub fn get_pending_actions(env: Env) -> Vec<PendingAction> {
        ...
        for action_id in 1..=counter {
            if let Some(action) = env
                .storage()
                .persistent()
                .get::<_, PendingAction>(&DataKey::PendingAction(action_id))
            {
